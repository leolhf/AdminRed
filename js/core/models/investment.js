...
function proyeccionInversion(inv) {
  ...
  const hoy      = new Date();
  const fechaStr = inv.fecha || inv.fechaCompra || fechaLocalISO(hoy);
...
function getCuotaEquipoCliente(cliente) {
  if (!cliente) return 0;
  // BUG FIX: antes solo comprobaba `typeof deudaEquipo === 'number'`, lo cual
  // es TRUE incluso cuando deudaEquipo ya es 0 (deuda liquidada). Por eso,
  // tras liquidarDeuda() —que pone deudaEquipo=0 pero no borra cuotaEquipo—
  // el sistema seguía cobrando la cuota de equipo junto al pago de megas
  // aunque el cliente ya no tuviera deuda pendiente.
  if (typeof cliente.deudaEquipo === 'number' && cliente.deudaEquipo > 0) {
    return cliente.cuotaEquipo || 0;
  }
  return 0;
}

// Progreso de recuperación de la deuda de equipo de un cliente.
// No existe un campo "total inicial" guardado: se reconstruye sumando
// lo ya cobrado (history[].montoEquipo de ese cliente) más lo que
// falta por cobrar (deudaEquipo actual).
function getProgresoEquipoCliente(cliente) {
  const restante = getDeudaEquipoCliente(cliente);
  if (!cliente) return { recuperado: 0, total: 0, restante: 0, pct: 0 };
  const recuperado = history
    .filter(h => h.id === cliente.id && h.montoEquipo)
    // BUG FIX: h.montoEquipo no se convertía a número. En datos históricos
    // importados como strings, la reducción concatenaba texto en vez de sumar,
    // produciendo porcentajes de progreso incorrectos (ej: "01000" en vez de 1000).
    .reduce((s, h) => s + Number(h.montoEquipo), 0);
  const total = recuperado + restante;
  const pct = total > 0 ? Math.round((recuperado / total) * 100) : 0;
  return { recuperado, total, restante, pct };
}

// BUG FIX: ambas funciones eran stubs vacíos (vincular devolvía false,
// desvincular era cuerpo vacío) — no modificaban investments en absoluto.
// Si algún módulo las llamaba esperando cambios persistentes, esos cambios
// se perdían silenciosamente. Ahora realizan la operación correcta sobre
// el array investments y guardan.
function vincularClienteAInversion(clienteId, investmentId) {
  const inv = investments.find(i => i.id === investmentId);
  if (!inv) return false;
  if (!Array.isArray(inv.clientesVinculados)) inv.clientesVinculados = [];
  if (inv.clientesVinculados.includes(clienteId)) return false; // ya vinculado
  inv.clientesVinculados.push(clienteId);
  save();
  return true;
}
...
function desvincularClienteDeInversion(clienteId, investmentId) {
  const inv = investments.find(i => i.id === investmentId);
  if (!inv || !Array.isArray(inv.clientesVinculados)) return;
  const antes = inv.clientesVinculados.length;
  inv.clientesVinculados = inv.clientesVinculados.filter(id => id !== clienteId);
  if (inv.clientesVinculados.length !== antes) save();
}
