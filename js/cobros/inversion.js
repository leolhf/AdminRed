...
function liquidarDeuda(id) {
  ...
  if(typeof registrarParaDeshacer==='function') registrarParaDeshacer(`Liquidar deuda de ${c.nombre}`);
  ...
  if(c.deudaEquipo<=0) { c.deudaEquipo=0; c.quiereLiquidar=false; c.cuotaEquipo=0; }
...
function openModalInversionPendiente() {
  ...
  const filasEquipo = conDeuda.map(c => {
    const p = getProgresoEquipoCliente(c);
    return `
    <div class="history-item">
      <div>
        ...
        ${p.total>0?`<div class="progreso-equipo" title="${fmt(p.recuperado)} de ${fmt(p.total)} recuperado">
