...
function actualizarTotalRebaja() {
  ...
  document.getElementById('rebaja-total').textContent = `−${fmt(total)}`;
}
...
function registrarRebaja() {
  ...
  const nota = document.getElementById('rebaja-nota').value;
  ...
  // Calcular el gasto
  const montoGasto = cantidad * valor;

  // Restar del inventario
  inv.cantidadAsignada += cantidad;
  inv.montoAsignado += montoGasto;
  ...
  const descGasto = `Rebaja: ${inv.desc} (${cantidad} ${inv.unidad==='m'?'m':'u'}) - ${motivoLabels[motivo]}`;
  ...
  renderSummary();
  closeRebajaModal();
  notify(`Rebaja registrada: ${cantidad} ${inv.unidad==='m'?'m':'u'} de ${inv.desc} — Gasto: ${fmt(montoGasto)}`);
}
