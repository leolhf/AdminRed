...
function checkMesNuevo() {
  ...
  if(config.mesActual && config.mesActual!==mesActual) {
    ...
    if(typeof showNotification === 'function') {
      showNotification('success', `🗓 Es ${label} — ¿Iniciar nuevo mes de cobros?`, 'Iniciar mes', iniciarNuevoMes);
    }
  }
}
...
function iniciarNuevoMes() {
  ...
  if(typeof guardarCheckpoint==='function') guardarCheckpoint('mes-nuevo'); // copia de seguridad justo antes del cambio
  const ahora=new Date();
  const mesActual=`${ahora.getFullYear()}-${String(ahora.getMonth()+1).padStart(2,'0')}`;
